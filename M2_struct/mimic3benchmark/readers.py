...
class Reader(object):
    def __init__(self, dataset_dir, listfile='listfile_30_short.csv'):
...
#         #     listfile_path = os.path.join(dataset_dir, "listfile.csv")
#         # else:
#         listfile_path = os.path.join(dataset_dir, "listfile_Readmission_days.csv")
#         with open(listfile_path, "r") as lfile:
#             self._data = lfile.readlines()
#         self._listfile_header = self._data[0]
#         self._data = self._data[1:]
...
class InHospitalMortalityReader_withLLM(Reader):
    def __init__(self, dataset_dir, listfile=None, period_length=48.0):
        """ Reader for in-hospital moratality prediction task.
        ...
        Reader.__init__(self, dataset_dir, listfile)
        self._data = [line.split(',') for line in self._data]
        self._data = [(x, int(y), int(llm)) for (x, y, llm) in self._data]
        self._period_length = period_length
