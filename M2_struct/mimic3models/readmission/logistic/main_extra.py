...
def load_extra_feature(path, names):
    df = pd.read_csv(path)
    return df["prediction"].values.astype(np.float32).reshape(-1, 1)
...
def main():
    ...
    parser.add_argument('--data', type=str, default=os.path.join(os.path.dirname(__file__), '../../../readmission/'))
    parser.add_argument('--output_dir', type=str, default='.')
    args = parser.parse_args()
    print(args)

    # Hardcoded paths to the prediction files
    extra_feature_file_train = os.path.join('/project/biocomplexity/hht9zt/mimic3-benchmarks/readmission/train', 'listfile_30_p.csv')
    extra_feature_file_test = os.path.join('/project/biocomplexity/hht9zt/mimic3-benchmarks/readmission/test', 'listfile_30_p.csv')
    ...
    print('Appending extra features ...')
    train_extra = load_extra_feature(extra_feature_file_train, train_names)
    val_extra = load_extra_feature(extra_feature_file_train, val_names)  # Using same file for val
    test_extra = load_extra_feature(extra_feature_file_test, test_names)
