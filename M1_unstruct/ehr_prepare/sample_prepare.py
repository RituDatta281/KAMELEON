...
DATASET = "mimic3"
TASK = 'readmission30' #or mortality

ehr_path = f"/{DATASET}_{TASK}_physician_summary.pkl"
agg_samples_path = f"/ehr_prepare/patient_{DATASET}_{TASK}_physician_summary.json"
...
for patient_id in tqdm(agg_samples.keys()):
    conditions_all, procedures_all, drugs_all, discharge_notes_all = [], [], [], []
    for i in range(len(agg_samples[patient_id])-1):
        ...
        #comment this out
        discharge_notes_all.append(agg_samples[patient_id][f'visit {i}']['physician_notes'])
    ...
    sample_new = {
        ...
        "drugs": drugs_all,
        "physician_notes": discharge_notes_all,
...
with open(f"/ehr_data/{DATASET}_{TASK}_samples_train_PN_summary.json", "w") as f:
    json.dump(samples_ehr_train, f)

with open(f"/ehr_data/{DATASET}_{TASK}_samples_val_PN_summary.json", "w") as f:
    json.dump(samples_ehr_val, f)
    
with open(f"/ehr_data/{DATASET}_{TASK}_samples_test_PN_summary.json", "w") as f:
