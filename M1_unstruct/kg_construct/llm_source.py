...
def build_kg_for_concepts_llm(concept_sets: List[Set[str]], save_interval: int = 1000) -> dict:
    ...
    for concept_set in tqdm(concept_sets, desc="Processing concept sets"):
        try:
        ...
        if (processed_count + skipped_count) % save_interval == 0:
            ...
            with open(f"/graph_PPID/kg_from_llm_{processed_count}.json", "w") as f:
                json.dump(list(kg_triples_set), f, indent=4)
            
    return kg_triples_set


def build_kg_for_concepts_llm_MT(concept_sets: List[Set[str]], save_interval: int = 1000) -> dict:
    kg_triples_set = set()
    processed_count = 0
    skipped_count = 0
...
def main():
    ...
    with open("/kg_construct/concepts/all_visit_concepts.json", "r") as f:
    ...
    with open("/kg_construct/kg_from_llm.json", "w") as f:
