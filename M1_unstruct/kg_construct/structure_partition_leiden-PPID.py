...
# from apis.unsloth_LLM import get_unsloth_response
    ...
    while retries < MAX_RETRIES:
        try:
            ...
            return response
             ...
             print('error in combining summaries')
...
def set_log_file(output_dir):
    ...
    log_file = os.path.join(output_dir, "community_summaries_pubmed.log")
...
def partition_and_summarize(g: nx.Graph, edge_relationships: dict, max_cluster_size: int, output_dir: str) -> None:
    ...
    while len(unique_communities) < 35:
        ...
        for level, node_to_community_map in node_to_community.items():
            ...
            # with ProcessPoolExecutor(max_workers=1) as executor:
                ...
                output_data = []
                for item in tqdm(unprocessed_communities, desc=f"Processing communities (run={run}, level={level})"):
                    community_data = process_community(item)  # Call function directly
                    output_data.append(community_data)
        ...
        save_communities(output_data, os.path.join(output_dir, "communities_Pubmed.json"))

    print("Summaries extracted and saved successfully.")

def main():
    kg_path = "/kg_construct/graph_PPID/kg_raw_pubmed.txt"
    output_dir = "/indexing"
    os.makedirs(output_dir, exist_ok=True)
    set_log_file(output_dir)

    if os.path.exists(os.path.join(output_dir, "edge_relationships_pubmed.pkl")) and os.path.exists(os.path.join(output_dir, "kg_pubmed.gpickle")):
        print("Combined knowledge graph and edge relationships already exist, reading...")
        with open( os.path.join(output_dir, 'kg_pubmed.gpickle'), 'rb') as f:
            g = pickle.load(f)
        with open(os.path.join(output_dir, "edge_relationships_pubmed.pkl"), 'rb') as f:
        ...
        with open(os.path.join(output_dir, "kg_pubmed.gpickle"), "wb") as f:
            pickle.dump(g, f)
        # nx.write_gpickle(g, os.path.join(output_dir, "kg.gpickle"))
        with open(os.path.join(output_dir, "edge_relationships_pubmed.pkl"), 'wb') as f:
