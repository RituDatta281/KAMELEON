...
# File paths
PATIENT_CONTEXT_PATH = f"patient_context/base_context/patient_contexts_{DATASET}_readmission30_notes.json"
PATIENT_CONTEXT_PATH_SUB = f"patient_context/base_context/patient_contexts_{DATASET}_readmission30_notes.json"
PATIENT_EMBEDDINGS_PATH = f"patient_context/base_context/patient_embeddings_{DATASET}_readmission30_notes.pkl"
PATIENT_DATA = f"ehr_prepare/pateint_{DATASET}_{TASK}_physician_summary.json"
...
# embedding_matrix = np.array([patient_embeddings[pid].cpu().numpy() for pid in patient_ids]).astype('float32')
embedding_matrix = np.array([
    patient_embeddings[pid].to(torch.float32).cpu().numpy()
...
embedding_matrix = embedding_matrix.squeeze(1)  # Removes the extra dimension
# print("Fixed embedding matrix shape:", embedding_matrix.shape)
...
for patient_id in tqdm(target_patient_ids):
    if patient_id not in patient_id_to_index or patient_id not in patient_data.keys():
    ...
    positive_ids = [pid for pid in similarity_scores if pid in patient_data and patient_data[pid]['label'] == label]

    negative_ids = [pid for pid in similarity_scores if pid in patient_data and patient_data[pid]['label'] != label]
...
output_path_k1 = f"patient_context/similar_patient/patient_to_top_1_patient_contexts_{DATASET}_{TASK}_notes.json"
output_path_k2 = f"patient_context/similar_patient/patient_to_top_2_patient_contexts_{DATASET}_{TASK}_notes.json"
