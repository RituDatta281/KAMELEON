...
def transform_patient_data(patient_data):
    ...
    for patient_id, visits in patient_data.items():
        ...
        for visit_num, visit_data in visits.items():
            ...
            output += "\nPhysician Notes:\n"
            for note in visit_data["physician_notes"]:
                output += f"{note}"
    ...
    for dataset in datasets:
        for task in tasks:
            ...
            if os.path.exists(f"{ehr_dir}/pateint_{dataset}_{task}_physician_summary.json"):
                ...
                sample_dataset_path = f"{ehr_dir}/{dataset}_{task}_physician_summary.pkl"
                ...
                with open(f"{ehr_dir}/pateint_{dataset}_{task}_physician_summary.json", "w") as f:
                    json.dump(patient_data, f, indent=4)
                    
            # Load or generate patient contexts
            out_path = f"patient_context/base_context/patient_contexts_{dataset}_{task}_notes.json"
